...
##FILTER=<ID=VAFC,Description=\"Variant allele frequency in tumor < 5%\">
...
### Print header:
### ### Change the columns to make sure tumor in column 9 (0 based)
my $tumor_temp_head = $head[$TUMOR];
$head[9] = $tumor_temp_head;
if($makehead ne "1")
{
	print join("\t", @head), "\n";
}
...
### variants with more than one alternative are still skipped e.g. chr12	19317131	.	GTT	GT,G	...
	if($splitIn[$TUMOR]=~/1\/0|0\/1|1\/1/)
	{
		...
		if($tumorGT=~/1\/0|0\/1|1\/1/)
		{
			...
			if($qual > 40 && $tumorDP >=10 && $tumorGQ>=20)	# All quality filters are OK
			{
				# Do nothing
			}
			elsif($qual > 20 && $tumorDP >=5 && $tumorGQ>=20)	# Medium quality values (not needed, is equal to bad values)(-2)
			{
				$confidence-=2;
			...
			if($tumorGT=~/1\/0|0\/1/)
			{
				if($sstumorGP[0] < $homreftum && $sstumorGP[2] < $tumaltgen)	# All genotype qualitys are OK
				{
					# Do nothing
				}
			...
			{
				if($sstumorGP[0] < $homreftum && $sstumorGP[1] < $tumaltgen)	# All genotype qualitys are OK
				{
					# Do nothing
				}
				else	# At least one genotype quality is bad (-2)
